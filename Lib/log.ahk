...
class logger {
    ...
    static LOG4AHK_G_MY_DLL_USE_MAP := map("cpp2ahk.dll", map("cpp2ahk", 0, "log_simple", 0, "log_simple_mt_color", 0,
        "set_console_transparency", 0, "log_set_format", 0))
    static log4ahk_load_all_dll_path() {
        ...
        for k, v in this.LOG4AHK_G_MY_DLL_USE_MAP {
            for k1, v1 in v {
                this.LOG4AHK_G_MY_DLL_USE_MAP[k][k1] := DllCall("GetProcAddress", "Ptr", DllCall("LoadLibrary", "Str",
                    k, "Ptr"), "AStr", k1, "Ptr")
            }
        }
    ...
    static log_out(para*) {
        ...
        if (!this.is_use_editor && this.is_out_console && !this.is_console_config) {
            ...
            consoleHandle := DllCall("CreateFile", "str", "CONOUT$", "int", 0x80000000 | 0x40000000, "int", 0x00000002,
                "int", 0, "int", 3, "int", 0, "int", 0)
        ...
        log_str :=  "| " this.log_strim log_str
        buf := this.strbuf(log_str, "utf-8")
        result := DllCall(this.LOG4AHK_G_MY_DLL_USE_MAP["cpp2ahk.dll"]["log_simple"], "ptr", buf, "int", this.log_mode,
            "int", this.is_out_file, "int", this.is_out_console, "cdecl int")
...
class Log4ahk_json {
    ...
    static parse(text, keepbooltype := false) {
        keepbooltype ? (_true := this.true, _false := this.false, _null := this.null) : (_true := true, _false := false,
            _null := "")
        NQ := "", LF := "", LP := 0, P := "", R := ""
        D := [C := (A := InStr(text := LTrim(text, " `t`r`n"), "[") = 1) ? [] : Map()], text := LTrim(SubStr(text, 2),
        " `t`r`n"), L := 1, N := 0, V := K := "", J := C, !(Q := InStr(text, '"') != 1) ? text := LTrim(text, '"') : ""
        loop parse text, '"' {
            Q := NQ ? 1 : !Q
                ...
                if (t := Trim(A_LoopField, " `t`r`n")) = "," || (t = ":" && V := 1)
                    ...
                    loop parse t {
                        ...
                        if InStr("`n`r `t", A_LoopField)
                            ...
                            C := A_LoopField = "[" ? [] : Map(), A ? D[L].Push(C) : D[L][K] := C, D.Has(++L) ? D[L] :=
                                C : D.Push(C), V := "", A := Type(C) = "Array"
                            continue
                        } else if InStr("]}", A_LoopField) {
                            if !A && V
                            ...
                            if RegExMatch(SubStr(t, A_Index), "m)^(null|false|true|-?\d+\.?\d*)\s*[,}\]\r\n]", &R) && (
                                ...
                                if A
                                    ...
                                    C[K] := R = "null" ? _null : R = "true" ? _true : R = "false" ? _false : IsNumber(R
                                    ) ? R + 0 : R, K := V := ""
        ...
        return J
        UC(S, e := 1) {
            static m := Map(Ord('"'), '"', Ord("a"), "`a", Ord("b"), "`b", Ord("t"), "`t", Ord("n"), "`n", Ord("v"),
            "`v", Ord("f"), "`f", Ord("r"), "`r", Ord("e"), Chr(0x1B), Ord("N"), Chr(0x85), Ord("P"), Chr(0x2029), 0,
            "", Ord("L"), Chr(0x2028), Ord("_"), Chr(0xA0))
            v := ""
            loop parse S, "\"
                if !((e := !e) && A_LoopField = "" ? v .= "\" : !e ? (v .= A_LoopField, 1) : 0)
                    v .= (t := InStr("ux", SubStr(A_LoopField, 1, 1)) ? SubStr(A_LoopField, 1, RegExMatch(A_LoopField,
                        "^[ux]?([\dA-F]{4})?([\dA-F]{2})?\K") - 1) : "") && RegexMatch(t, "i)^[ux][\da-f]+$") ? Chr(Abs(
                            "0x" SubStr(t, 2))) SubStr(A_LoopField, RegExMatch(A_LoopField,
                                "^[ux]?([\dA-F]{4})?([\dA-F]{2})?\K")) : m.has(Ord(A_LoopField)) ? m[Ord(A_LoopField)] SubStr(
                                    A_LoopField, 2) : "\" A_LoopField, e := A_LoopField = "" ? e : !e
            return v
        }
    }
    ...
    static stringify(obj, expandlevel := unset, space := "  ", unicode_escaped := false) {
        ...
        CO(O, J := 0, R := 0, Q := 0) {
            ...
            if (OT := Type(O)) = "Array" {
                ...
                for key, value in O {
                    ...
                    D .= (J > R ? "`n" CL(R + 2) : "") (F ? (%F%1 (Z ? "" : CO(value, J, R + 1, F)) %F%2) : ES(value, J,
                        unicode_escaped)) (OT = "Array" && O.Length = A_Index ? E : C)
                }
            } else {
                D := !R ? M1 : ""
                for key, value in (OT := Type(O)) = "Map" ? (Y := 1, O) : (Y := 0, O.OwnProps()) {
                    ...
                    Z := VT = "Array" && value.Length = 0 ? "[]" : ((VT = "Map" && value.count = 0) || (VT = "Object" &&
                        ObjOwnPropCount(value) = 0)) ? "{}" : ""
                    D .= (J > R ? "`n" CL(R + 2) : "") (Q = "S" && A_Index = 1 ? M1 : E) ES(key, J, unicode_escaped) K (
                        F ? (%F%1 (Z ? "" : CO(value, J, R + 1, F)) %F%2) : ES(value, J, unicode_escaped)) (Q = "S" &&
                            A_Index = (Y ? O.count : ObjOwnPropCount(O)) ? M2 : E) (J != 0 || R ? (A_Index = (Y ? O.count :
        ...
        ES(S, J := 1, U := false) {
            static ascii := Map("\", "\", "`a", "a", "`b", "b", "`t", "t", "`n", "n", "`v", "v", "`f", "f", "`r", "r",
                Chr(0x1B), "e", "`"", "`"", Chr(0x85), "N", Chr(0x2029), "P", Chr(0x2028), "L", "", "0", Chr(0xA0), "_"
            )
            switch Type(S) {
                case "Float":
                    if (v := '', d := InStr(S, 'e'))
                ...
                case "String":
                    ...
                    if (U && RegExMatch(S, "m)[\x{7F}-\x{7FFF}]")) {
                        loop parse S
                            v .= ascii.Has(A_LoopField) ? "\" ascii[A_LoopField] : Ord(A_LoopField) < 128 ? A_LoopField :
                                "\u" format("{1:.4X}", Ord(A_LoopField))
                        return '"' v '"'
                    } else {
                        loop parse S
